...
<!--     <form action='hj_delete_clip' method='GET'  id=frm1> 
    <input type=hidden id=my_id name="my_id" value="">
    </form> -->
    <c:forEach items="${ij_show_my_plan2}" var="dto">
        ...
        <tr>
            <td class="yplan_td2">
                  <div><input type=button id=planlist value='일정보기' my_id="${dto['my_id']}" my_city="${dto['my_city']}" >
                     ${dto['my_start']}<br>
                     ${dto['my_name']}<br>
                   <div><input type=button id=planlist_del value='일정삭제' plan_id="${dto['my_id']}">
                     ${dto['my_city']}<br>
                     ${dto['my_place']}<br>
                  </div></div>
            </td>
...
$(document).ready(function () {
   ...
   .on('click','#planlist_del',function(){
      $.ajax({
              method:'GET',
               url:'hj_delete_clip',
               datatype:'text',
          	 ...
          	 setTimeout(function(){
                 location.href='ij_show_my_plan2'
			 },200)
   })
</script>  
</html>